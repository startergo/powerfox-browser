...
var gPowerFoxSSUAManager = {
  ...
  _view: {
    ...
    getCellText: function (aRow, aColumn)
    {
      if (aColumn.id == "domainCol")
        return gPowerFoxSSUAManager._SSUAs[aRow].domain;
      else if (aColumn.id == "uaCol")
        return gPowerFoxSSUAManager._SSUAs[aRow].ua;
      return "";
    },
  ...
  onLoad: function ()
  {
    this._bundle = document.getElementById("powerFoxPreferences");
  ...
  init: function (aParams)
  {
    ...
    this._prefBranch = Services.prefs.getBranch(UA_PREF_BRANCH);
    ...
    treecols.addEventListener("click", event => {
      ...
      gPowerFoxSSUAManager.onSSUAsort(sortField);
    });
  ...
  fillUA: function(popup)
  {
    if (popup.value == "")
      ...
      document.getElementById("ua").value = gPowerFoxPane.validUA[popup.value];
    this.onHostInput();
  },
  ...
  _loadSSUAs: function ()
  {
    ...
    let count = 0;
    let domains = this._prefBranch.getChildList("");
    for (let domain of domains) {
      this._addSSUAToList(domain, this._prefBranch.getCharPref(domain));
    }
...
{
  gPowerFoxSSUAManager.init(aParams);
}
